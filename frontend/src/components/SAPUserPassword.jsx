...
// 1. token prop'unu ekledik
export default function SAPUserPassword({ userId, token }) { 
  ...
  const handleReveal = async () => {
    try {
      const response = await fetch(`http://localhost:8000/sap-users/${userId}/reveal-password`, {
        method: 'POST',
        // 2. Token'ı backend'e gönderiyoruz
        headers: {
      ...
      if (!response.ok) {
        // 3. Backend'den gelen GERÇEK hata mesajını yakalıyoruz
  ...
  const handleCopy = async () => {
    try {
      const response = await fetch(`http://localhost:8000/sap-users/${userId}/copy-password`, {
        method: 'POST',
        // Token'ı backend'e gönderiyoruz
        headers: {
          'Authorization': `Bearer ${token}` 
        }
      });
